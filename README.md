...
For local editing, create a symlink from your local clone of this repository to the `Catppuccin` subdirectory of the `Packages` directory (revealed through the **Preferences** > **Browse Packages...** menu in Sublime Text). On macOS, this looks like `ln -s $(realpath .) '/Users/$USER/Library/Application Support/Sublime Text/Packages'`. Uninstall this theme from Package Control if installed via that method. Sublime Text should now automatically reload the theme files in `build/` on change.
